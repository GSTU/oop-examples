...
class GhostProgressbar : ThemeControl154
{
    ...
    protected override void PaintHook()
    {
        ...
        cblend.Positions[0] = 0;
        cblend.Positions[1] = 1;
        int tempwidth = Convert.ToInt32(((Width / _Maximum) * _Value) - 2);
        if (tempwidth <= 0)
            tempwidth = 1;
        DrawGradient(cblend, new Rectangle(0, 0, tempwidth, Height - 2));
        cblend.Colors[1] = Color.FromArgb(80, 80, 80);
        DrawGradient(cblend, new Rectangle(0, 0, tempwidth, Height / 5 * 2));
...
class GhostControlBox : ThemeControl154
{
    ...
    protected override void PaintHook()
    {
        ...
        //Draw icons
        G.DrawString("0", new Font("Marlett", 10), Brushes.White, new Point(5, 3));
        ...
        G.DrawRectangle(BEdge, new Rectangle(new Point(1, 1), new Size(24, 16)));
        ...
        switch (State)
        {
            case MouseState.Over:
                if (X > 0)
                {
                    G.FillRectangle(new SolidBrush(Color.FromArgb(20, Color.White)), new Rectangle(new Point(0, 1), new Size(25, Height - 2)));
                }
                break;
            case MouseState.Down:
                if (X > 0)
                {
                    G.FillRectangle(new SolidBrush(Color.FromArgb(20, Color.Black)), new Rectangle(new Point(0, 1), new Size(25, Height - 2)));
                    FindForm().Hide();
                }
                break;
        }
