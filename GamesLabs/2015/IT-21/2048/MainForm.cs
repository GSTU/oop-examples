    ...
    public partial class MainForm : Form
    {
        ...
        public void XSDCheck()
        {
            try
            {
                ...
                XMLSettings.ValidationEventHandler += new ValidationEventHandler(Validation);
                ...
                if (valid == true)
                {
                    ...
                    xml.Load("XML.xml");
                    this.Text = xml["GameSetting"]["GameTitle"].InnerText;
                    string GameOwerForeColor = xml["GameSetting"]["GameColor"]["FormColor"].InnerText;
                    string backgroundBackground = xml["GameSetting"]["GameColor"]["BackgroundColor"].InnerText;
                    GameOwer.BackgroundImage = new Bitmap(GameOwerForeColor);
                    bg.BackgroundImage = new Bitmap(backgroundBackground);
                    x = xml["GameSetting"]["GameConfig"]["Location"]["X"].InnerText;
                    y = xml["GameSetting"]["GameConfig"]["Location"]["Y"].InnerText;
                    s = xml["GameSetting"]["GameConfig"]["Size"].InnerText;
                }
                else
                {
                    this.Text = "Нет XML данных!";
                    GameOwer.BackgroundImage = new Bitmap("gameover2.png");
                    bg.BackgroundImage = new Bitmap("Green.png");
                    x = "0"; y = "40"; s = "278";
                    MessageBox.Show("XML файл повреждён или не найден!\nЗагружены стандартные параметры.");
                }
            }
            catch (Exception Ex)
            {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            this.Focus();
            background.Location = new System.Drawing.Point(int.Parse(x), int.Parse(y));
            background.Size = new System.Drawing.Size(int.Parse(s), int.Parse(s));
            background.Name = "bk";
                    ...
                    if (num_matrix[i, j] != 0 && num_matrix[i, j] != null)
                    {
                        for (int k = j + 1; k < num_matrix.GetLength(0); k++)
                        {
                            ...
                            if (num_matrix[i, k] == num_matrix[i, k - 1] && !stack[i, k] && !stack[i, k - 1])
                            {
                                ...
                                num_matrix[i, k - 1] = 0;
                                stack[i, k] = true;
                                this.Text = scores.Text = (Convert.ToInt32(scores.Text) + num_matrix[i, k]).ToString();
                                can_spawn = true;
                            }
                        }
                    }
                    ...
                    if (num_matrix[i, j] != 0 && num_matrix[i, j] != null)
                    {
                        for (int k = i - 1; k >= 0; k--)
                        {
                            ...
                            if (num_matrix[k, j] == num_matrix[k + 1, j] && !stack[k, j] && !stack[k + 1, j])
                            {
                                ...
                                num_matrix[k + 1, j] = 0;
                                stack[k, j] = true;
                                this.Text = scores.Text = (Convert.ToInt32(scores.Text) + num_matrix[k, j]).ToString();
                                can_spawn = true;
                            }
                        }
                    }
